...
The graph hands it the answer up front through one MCP tool, `inspect_typescript_source_flow`. The agent chooses one request branch per call: `overview`, `entrypoints`, `lookup`, `trace`, `details`, or `escape`. The agent stops fanning out across files.
