...
`)
	...
	createAt := strings.Index(text, `"name": "ShoppingOrderProvider.create"`)
	atAt := strings.Index(text, `"name": "ShoppingOrderProvider.at"`)
	if createAt < 0 {
		t.Fatalf("query_nodes did not return ShoppingOrderProvider.create:\n%s", text)
	}
	classAt := strings.Index(text, `"name": "ShoppingOrderProvider"`)
