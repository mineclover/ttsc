...
`)
	...
	// Exact name still reaches it as an index node, without returning source.
	exact := toolText(t, server, `{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"query_nodes","arguments":{"query":"ZzzGeneratedClient"}}}`)
	if !strings.Contains(exact, "ZzzGeneratedClient") || strings.Contains(exact, "run(): number") {
		t.Fatalf("exact-name query did not return a compact generated class index:\n%s", exact)
	}
}
