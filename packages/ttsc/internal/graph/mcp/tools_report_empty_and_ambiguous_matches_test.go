...
`)
	...
	if !strings.Contains(clean, `"total": 0`) || !strings.Contains(clean, `"diagnostics": []`) {
		t.Fatalf("query_diagnostics did not return an empty diagnostic list for a clean file:\n%s", clean)
	}
