...
// TestGraphExploreReturnsSourceAndBlastRadius verifies query_nodes answers a
// structural question in one round-trip: it returns the matched node's
// checker-resolved incoming call edge and a blast-radius count of transitive
// dependents without dumping source bodies.
//
//  1. Compile a call chain top() -> mid() -> leaf(), each in its own file.
//  2. Explore "leaf".
//  3. Assert the response carries the incoming value-call edge from mid and a
//     blast radius of 2 (mid and top transitively depend on leaf).
...
`)
	...
	if strings.Contains(text, "return 1") {
		t.Fatalf("query_nodes returned source body instead of index data:\n%s", text)
	}
	...
	if !strings.Contains(text, midUse) || !strings.Contains(text, `"line": 3`) {
		t.Fatalf("query_nodes did not show the incoming callsite line from mid:\n%s", text)
	}
	if !strings.Contains(text, `"dependents": 2`) {
