...
// highly connected, otherwise dominates ranking and floods responses). They stay
// in the program for type resolution and remain reachable as edge targets and by
// exact name; they just do not rank as primary matches. Empty when cwd is unset,
// the tree is not a git work tree, or git is unavailable, so a non-git project is
// unaffected.
func (s *Server) gitIgnoredFiles() map[string]bool {
	if s.cwd == "" {
		return nil
	}
	...
	cmd := exec.Command("git", "-C", s.cwd, "check-ignore", "--stdin")
