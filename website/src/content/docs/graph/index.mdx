...
A tree-sitter graph has no types and resolves edges by guessing from text. An editor's language server has types but no architecture projection. Only a graph built on the real checker holds the resolved structure and the affected set in one place — and every node and edge declares its provenance and confidence, so an agent never mistakes inference for fact.
