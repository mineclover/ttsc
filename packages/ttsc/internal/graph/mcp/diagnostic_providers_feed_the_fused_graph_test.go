...
`)
	...
	if !strings.Contains(text, `"errors": 1`) || !strings.Contains(text, `"total": 1`) {
		t.Fatalf("query_nodes did not count the injected provider diagnostic on target:\n%s", text)
	}
	if strings.Contains(text, "TS9999") {
		t.Fatalf("query_nodes leaked plugin diagnostic detail into the index:\n%s", text)
	}
}
