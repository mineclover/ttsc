...
func collapsedHandle(t *testing.T, text string) (string, string) {
	...
	if err := json.Unmarshal([]byte(text), &payload); err != nil {
		t.Fatalf("query_nodes returned invalid JSON:\n%s", text)
	}
	for _, node := range payload.Nodes {
		if !strings.HasPrefix(node.Name, "clusterTarget") || node.Handle == "" {
			continue
		}
		marker := node.Name + "-body-marker"
		if strings.Contains(text, marker) {
			continue
		}
		return node.Handle, marker
	}
...
func nodeHandleFromText(t *testing.T, text string, prefix string) string {
	t.Helper()
	name := prefix
	if fields := strings.Fields(prefix); len(fields) > 1 {
