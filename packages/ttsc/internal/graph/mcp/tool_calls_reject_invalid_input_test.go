...
// that are deliberately not faults: query_exports limit 0 means "count without
// records", and a blank query_diagnostics file means "the whole project".
//
...
`)
	...
	summaryOnly := toolStructured(t, server, `{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"query_exports","arguments":{"limit":0}}}`)
	exports, _ := summaryOnly["exports"].([]any)
	page, _ := summaryOnly["page"].(map[string]any)
	if len(exports) != 0 || page["totalRecords"] != float64(1) {
		t.Fatalf("query_exports limit 0 did not return count without records:\n%v", summaryOnly)
	}
	...
	badPage := errorOf(t, server, `{"jsonrpc":"2.0","id":4,"method":"tools/call","params":{"name":"query_exports","arguments":{"page":-1}}}`)
	if badPage["code"] != float64(-32602) {
		t.Fatalf("bad query_exports page code was not -32602: %v", badPage["code"])
	}
	if msg, _ := badPage["message"].(string); !strings.Contains(msg, "page") {
		t.Fatalf("bad query_exports page message did not name the fault: %v", badPage["message"])
	}
