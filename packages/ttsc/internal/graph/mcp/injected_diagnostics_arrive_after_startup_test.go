...
`)
	...
	if !strings.Contains(after, `"errors": 1`) {
		t.Fatalf("late-arriving injected diagnostic was not picked up on the next query:\n%s", after)
	}
	diagnostics := toolText(t, server, `{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"query_diagnostics","arguments":{}}}`)
	if !strings.Contains(diagnostics, "lint/late-rule") {
		t.Fatalf("query_diagnostics did not include late-arriving injected diagnostic:\n%s", diagnostics)
	}
}
